    ...
    public partial class frmOptions : Form
    {
        ...
        private void SaveRegistrySettings()
        {
            ...
            if (objRegistry != null)
            {
                ...
                string useAI = cbUseLoggingApplicationInsights.SelectedItem.ToString();
        ...
        private void PopulateGlobalOptions()
        {
            ...
            Array enumvalues = Enum.GetValues(typeof(UseAILogging));
            List<ListItem> lstUseAIOptions = new List<ListItem>();
            foreach (object myoption in enumvalues)
            {
                ListItem listitem = new ListItem(myoption, LanguageManager.GetString("String_ApplicationInsights_" + myoption, _strSelectedLanguage));
        ...
        private void clbPlugins_VisibleChanged(object sender, EventArgs e)
        {
            ...
            using (new CursorWait(false, this))
            {
                foreach (Plugins.IPlugin plugin in Program.PluginLoader.MyPlugins)
                {
                    ...
                    if (GlobalOptions.PluginsEnabledDic.TryGetValue(plugin.ToString(), out bool check))
                    {
        ...
        private void cbUseLoggingApplicationInsights_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (useAI > UseAILogging.Info)
            {
                ...
                DialogResult result = Program.MainForm.ShowMessageBox(msg, "Really enable detailed upload?", MessageBoxButtons.OKCancel);
