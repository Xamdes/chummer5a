...
namespace Chummer.Plugins
{
    ...
    public class PluginHandler : IPlugin
    {
        ...
        public override string ToString() => "SINners";
        ...
        private bool HandleLoadCommand(string argument)
        {
            ...
            Thread thread = new Thread((myargument) =>
            {
                if (!blnHasDuplicate)
                {
        ...
        IEnumerable<TabPage> IPlugin.GetTabPages(frmCareer input)
        {
            ...
            Task<CharacterExtended> ce = uc.SetCharacterFrom(input);
        ...
        public static SINner MySINnerLoading
        {
            get; internal set;
        }
        public NamedPipeManager PipeManager
        {
            get; private set;
        }
        ...
        public static async void MyOnSaveUpload(object sender, Character input)
        {
            try
            {
                ...
                using (new CursorWait(true, MainForm))
                {
                    ...
                    CharacterShared found = (from a in MainForm.OpenCharacterForms where a.CharacterObject == input select a)
                        .FirstOrDefault();
        ...
        private async void mnuSINnersArchetypes_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (new CursorWait(true, MainForm))
                {
                    ...
                    ResultGroupGetSearchGroups result =
                    ...
                    if (result.CallSuccess == true)
                    {
                        ...
                        SINSearchGroupResult ssgr1 = ssgr;
                        PluginHandler.MainForm.CharacterRoster.DoThreadSafe(() =>
                        {
                            using (new CursorWait(true, MainForm))
                            {
                                ...
                                {
                                    List<SINnerSearchGroup> list = ssgr1.SinGroups.Where(a => a.Groupname == "Archetypes").ToList();
        ...
        public Assembly GetPluginAssembly() => typeof(ucSINnersUserControl).Assembly;
        ...
        public System.Windows.Forms.UserControl GetOptionsControl() => new ucSINnersOptions();
        ...
        public async Task<IEnumerable<TreeNode>> GetCharacterRosterTreeNode(frmCharacterRoster frmCharRoster, bool forceUpdate)
        {
            try
            {
                ...
                using (new CursorWait(true, frmCharRoster))
                {
                    frmCharRoster.DoThreadSafe(() =>
                    {
                        ...
                        List<ToolStripMenuItem> menulist = myContextMenuStrip.Items.Cast<ToolStripMenuItem>().ToList();
                        foreach (ToolStripMenuItem item in menulist)
                        {
                            switch (item.Name)
                            {
                                case "tsToggleFav":
                    ...
                    if (ChummerHub.Client.Properties.Settings.Default.UserModeRegistered == true)
                    {
                        ...
                        Func<Task<HttpOperationResponse<ResultAccountGetSinnersByAuthorization>>> myMethodName = async () =>
                        {
                            ...
                            {
                                SINnersClient client = StaticUtils.GetClient();
                                HttpOperationResponse<ResultAccountGetSinnersByAuthorization> ret = await client.GetSINnersByAuthorizationWithHttpMessagesAsync();
                                return ret;
                            }
                            catch (Exception e)
                            {
                        ...
                        IEnumerable<TreeNode> res = await ChummerHub.Client.Backend.Utils.GetCharacterRosterTreeNode(forceUpdate, myMethodName);
                        if (res == null)
                        {
                    ...
                    List<KeyValuePair<string, TreeNode>> myadd = MyTreeNodes2Add.ToList();
                    List<KeyValuePair<string, TreeNode>> mysortadd = (from a in myadd orderby a.Value.Text select a).ToList();
                    foreach (KeyValuePair<string, TreeNode> addme in mysortadd)
                    {
            ...
            catch (Exception e)
            {
                Log.Error(e);
                TreeNode node = new TreeNode("SINners Error: please log in") { ToolTipText = e.ToString(), Tag = e };
                frmCharacterRoster.CharacterCache objCache = new frmCharacterRoster.CharacterCache
                {
        ...
        private void AddContextMenuStripRecursive(List<TreeNode> list, ContextMenuStrip myCmsRoster)
        {
            foreach (TreeNode node in list)
            {
                ...
                {
                    List<TreeNode> myList = node.Nodes.Cast<TreeNode>().ToList();
                    AddContextMenuStripRecursive(myList, myCmsRoster);
                }
            }
        }

        public bool BlnHasDuplicate
        {
            get; set;
        }
                ...
                if (argument.StartsWith("Load:"))
                {
                    try
                    {
                        ...
                        if (Guid.TryParse(SINnerIdvalue, out Guid SINnerId))
                        {
                            ...
                            HttpOperationResponse<ResultSinnerGetSINById> found = await client.GetSINByIdWithHttpMessagesAsync(SINnerId);
        ...
        private static async Task<Character> MainFormLoadChar(string fileToLoad)
        {
            ...
            using (frmLoading frmLoadingForm = new frmLoading { CharacterFile = fileToLoad })
            {
                ...
                //already open
                IEnumerable<Character> foundseq = (from a in PluginHandler.MainForm.OpenCharacters
                ...
                PluginHandler.MainForm.DoThreadSafe(() =>
                {
                    IEnumerable<CharacterShared> foundform = from a in PluginHandler.MainForm.OpenCharacterForms
