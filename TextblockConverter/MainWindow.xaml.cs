    ...
    public partial class MainWindow
    {
        ...
        private void ConvertString(bool critter = true)
        {
            ...
            for (int i = 2; i < lines.Length; i++)
            {
                ...
                {
                    string rawMove = lines[i].Replace("Movement ", string.Empty);
                    if (rawMove.Contains('('))
                    {
                        rawMove = rawMove.Replace('(', ',').Replace(")", string.Empty);
                    }
                    string[] movements = rawMove.Split(',').ToArray();
                    string[] walk = new[] { "0", "1", "0" };
                    string[] run = new[] { "0", "0", "0" };
                    string[] sprint = new[] { "0", "1", "0" };
                ...
                {
                    string line = lines[i].Replace("Physical Skills ", string.Empty).Replace("Matrix Skills ", string.Empty).Replace("Magic Skills ", string.Empty).Replace("Skills ", string.Empty);
                ...
                {
                    string line = lines[i].Replace("Complex Forms ", string.Empty);
                    string[] split = line.Split(',');
                ...
                {
                    string line = lines[i].Replace("Powers ", string.Empty);
                    string[] split = line.Split(',');
                ...
                {
                    string line = lines[i].Replace("Programs ", string.Empty);
                    string[] split = line.Split(',');
                ...
                {
                    string line = lines[i].Replace("Qualities ", string.Empty);
                    string[] split = line.Split(',');
                ...
                {
                    string line = lines[i].Replace("Armor ", string.Empty);
